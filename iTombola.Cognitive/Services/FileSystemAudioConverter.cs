...
namespace iTombola.Cognitive.Services
{
    public class FileSystemAudioConverter : IAudioConverter
    {
        ...
        public async Task<AudioConverterResponse> ConvertDescriptionToAudio(NumberDescriptionInfo description, string voiceName, CancellationToken token = default)
        {
            ...
            {
                result.AudioFilePath = await SynthetizeTextAsync(description.Content, description.Culture, description.Type, voiceName, token);
            }
            return result;
        }

        public async Task<AudioConverterResponse> ConvertTextToAudio(string text, string culture, string voiceName, CancellationToken token = default)
        {
            var result = new AudioConverterResponse
            {
                Audio = null,
            ...
            {
                result.AudioFilePath = await SynthetizeTextAsync(text, culture, DescriptionType.Text, voiceName, token);
            }
            return result;
        }

        private async Task<string> SynthetizeTextAsync(string text, string culture, DescriptionType type, string voiceName, CancellationToken token)
        {
            ...
            else if (!string.IsNullOrWhiteSpace(config.VoiceName))
