...
namespace iTombola.Core.Implementations
{
	public class TombolaService : ITombolaService
	{
		...
		public async Task<ExtractedNumbersResponse> ExtractNumberFromStream(Stream imageData, string culture, string dialect,
			double expectedConfidence, int maximumResultNumber = 1, CancellationToken token = default)
		{
...

			{
				...
				foreach (var number in validNumbers)
				{
					...
					var audioConverterResult = await this.audioConverter.ConvertTextToAudio(number.Number, culture, token);
					...
					{
						audioConverterResult = await this.audioConverter.ConvertDescriptionToAudio(number.Description, token);
						if (audioConverterResult.Audio != null)
